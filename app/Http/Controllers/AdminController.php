...
class AdminController extends Controller
{
    ...
    public function admin_view_result_api($id)
    {

        $std_exam = user_exam::where('id', $id)->firstOrFail();
        $data['student_info'] = User::where('id', $std_exam->user_id)->firstOrFail();
        $data['exam_info'] = Oex_exam_master::where('id', $std_exam->exam_id)->firstOrFail();
        $data['result_info'] = Oex_result::where('exam_id', $std_exam->exam_id)->where('user_id', $std_exam->user_id)->firstOrFail();
        $data['question'] = Oex_question_master::where('exam_id', $id)->get()->toArray();

        return response()->json($data);
    }

    public function registered_students_api()
    {
